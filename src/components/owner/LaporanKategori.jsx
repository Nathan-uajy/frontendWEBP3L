...
const LaporanKategori = ({ setToast }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('❌ Gagal ambil data laporan:', err.response?.data || err.message);
  ...
  const downloadPDF = () => {
    const img = new Image();
    img.src = '/images/reusemart-logo.jpg'; // Pastikan file ini ada di public/images
  ...
  return (
    <div>
      <Button variant="success" onClick={fetchData}>
        ⬇️ Unduh & Preview Laporan
      </Button>

      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title>📊 Data Laporan Penjualan per Kategori</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={downloadPDF}>📥 Simpan PDF</Button>
        </Modal.Footer>
      </Modal>
